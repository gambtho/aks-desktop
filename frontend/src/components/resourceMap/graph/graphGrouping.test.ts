...
describe('groupGraph', () => {
  ...
  it('groups nodes by node', () => {
    ...
    // Unscheduled group has 3 members so it gets a higher weight boost
    expect(nodeNames).toEqual(['Node-Unscheduled', 'Node-node1']);
  });
  ...
  it('does not link a kubeObject to the Unscheduled group', () => {
    ...
    const unscheduledGroup = groupedGraph.nodes?.find(n => n.id === 'Node-Unscheduled');
    expect(unscheduledGroup).toBeDefined();
    expect(unscheduledGroup?.label).toBe('Unscheduled');
