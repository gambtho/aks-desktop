...
describe('KNOWN_FEATURE_TYPES invariants', () => {
  // Guard against accidental sync divergence with TELEMETRY_EVENT_ALLOWLIST in setup.ts.
  // PLUGINS_LOADED and ERROR_BOUNDARY are intentionally omitted — they route to dedicated
  // handlers. Adding either here would cause double-emission.
  it('does not include headlamp.plugins-loaded (routes to trackPluginsLoaded, not trackFeature)', () => {
    expect(KNOWN_FEATURE_TYPES.has('headlamp.plugins-loaded')).toBe(false);
  });
  it('does not include headlamp.error-boundary (captured by TelemetryErrorBoundary directly)', () => {
    expect(KNOWN_FEATURE_TYPES.has('headlamp.error-boundary')).toBe(false);
  });
});
