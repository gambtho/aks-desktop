...
type KnownAksTier = 'Free' | 'Standard' | 'Premium';
export type AksTier = KnownAksTier | 'Unknown';
const VALID_AKS_TIERS: ReadonlySet<KnownAksTier> = new Set(['Free', 'Standard', 'Premium']);

export function sanitizeTier(tier: string | null | undefined): AksTier {
  return tier && VALID_AKS_TIERS.has(tier as KnownAksTier) ? (tier as KnownAksTier) : 'Unknown';
}
