...
import type { InstallId } from './installId';
import {
  type AksTier,
...
export interface InitTelemetryOptions {
  ...
  /** Anonymous per-install UUID from {@link getOrCreateInstallId}. Never pass PII here. */
  installId: InstallId;
...
export function isTelemetryAttempted(): boolean {
...
export interface ClusterShapeInput {
  ...
  nodeCount: number | null | undefined; // 0 is valid data (not absent)
  namespaceCount: number | null | undefined; // 0 is valid data (not absent)
  region: string | null | undefined;
  /** Raw ARM tier string — sanitized to AksTier on emit */
 ...
 * cluster across re-renders or capability refreshes (once per session
 * per resource-id, regardless of data changes).
 */
